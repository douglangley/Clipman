...
Implementation checkpoint, 2026-08-11: phases 1–4 and the first implementation slices of phases 5–7 are committed on `go-server`. The server core, administration, onboarding, certificates, updater transaction core, native wrapper launch paths, Linux helper migration, and the first differential/security gates are implemented. The legacy Python updater now provides the bridge into native Linux assets with same-version migration, transition-ZIP fallback, manifest-v2/digest validation, safe tar extraction, and rollback to the preserved Python launcher. The full historical-upgrade simulations, remaining differential/endurance gates, manual recovery documentation, and deferred native build/package matrices remain. No bridge release has been published.
