...
Revision: 2026-08-13 — native package-mode and historical bridge fixture verification
...
Release-layout checkpoint, 2026-08-14: the release builder now stages one server-only `ClipmanServer-<version>` directory with self-contained `windows-amd64`, `macos-universal`, `linux-amd64`, `linux-arm64`, and `linux-armv7` directories. Each exposes `clipmanserver`, includes its installer, manifest, checksums, documentation, and server support files, and is the source for the existing platform-native update archive names. The separate Clipman CLI build remains solely responsible for client binaries and documentation. Server compatibility aliases retain `clipman-server`, `Clipman Server.exe`, and `Clipman Server.app`; installed settings/service/data paths are unchanged. The combined `ClipmanServer-<version>.zip` remains separately generated with its historical Python layout until the bridge window closes.

Local macOS build checkpoint, 2026-08-23: `ClipmanServerMac/Scripts/package-release.sh` successfully rebuilt the declared server version 2.6.2 into `ClipmanServer-macOS-universal-2.6.2.zip` on Apple Silicon with Go 1.26.5 and Apple Swift 6.3.3. Verification extracted the produced ZIP rather than trusting only the staging bundle: the Swift wrapper and embedded Go core both reported 2.6.2, both contained `x86_64` and `arm64` slices, `LSMinimumSystemVersion` was 13.0, strict nested-signature verification passed, and the ZIP SHA-256 was `e22c861a8be95f8650a8942a4593876e255b236e9c4f5e08ca8397e227fa2215` for 5,861,992 bytes. This is an ad-hoc-signed development artifact, not a distributable release; Developer ID signing, notarization, Gatekeeper, interactive menu/login-item checks, Intel execution, and installed-app update/rollback remain release gates.
...
The release build also leaves an inspectable per-platform directory before archive publication, following the Clipman CLI build's directory organization without including the CLI itself:
...
`clipmanserver` is the platform server-management entrypoint. `clipman-server`, `Clipman Server.exe`, and `Clipman Server.app` remain server compatibility names. No `clipman` or `clipman-cli` binary is included; those remain products of the separate Clipman CLI build. Platform archives are derived from the matching directory; the combined Python transition ZIP remains a separate build output rather than being confused with the native layout.
...
macOS checkpoint, updated 2026-08-23: standalone native and combined transition ZIPs were built and extracted on Apple Silicon; their nested signatures, arm64/x86_64 slices, wrapper/core versions, manifests, and archive integrity passed. A fresh build from current `go-server` again verified the extracted universal wrapper/core, version agreement, macOS 13 floor, strict ad-hoc nested signature, archive integrity, byte size, and SHA-256 recorded at the top of this plan. The packaged core passed live health, conditional upload/download, backup compatibility, setup-link, inventory/deletion, private-CA HTTPS, fingerprint, and one-download CA-sharing checks. The full Go suite, vet, race detector, both Swift architecture type checks, shell syntax, and diff checks pass. Intel execution, full packaged `clipman-cli` corpus, login-item/UI automation, production Developer ID signing/notarization/Gatekeeper, and an actual installed-app forced rollback remain outstanding.
...
Implementation status: updater security/transaction core, standalone updater command, manifest v2, native asset selection, same-version migration selection, health rollback, and simulated service coordination are implemented. Compatibility `package` mode exercises an installed endpoint through the real CLI, and a Windows package sandbox passes before and after a forced health rollback with byte-identical persistent state. Concrete installed-service adapters and package-script integration are completed alongside Phase 5.
...
Implementation status: in progress. Native Windows/macOS launch paths and the Python/OpenSSL-free Docker runtime are implemented. Native Linux release archives and the macOS universal archive are generated from the server-only release layout; the combined transition archive remains available for historical updaters. The Swift updater has transactional replacement plus post-launch server-health rollback. Linux user/system helpers use native settings queries and the native updater, including compatible deployed flags, HTTPS discovery, zip/tar.gz validation, offline maintenance, and persistent user-systemd startup through lingering enablement and status reporting. Clean Windows wrapper packaging, production macOS signing/notarization, clean-machine desktop runs, native Windows/macOS certificate-address selection, and the remaining full platform package/update matrices remain.
...
Implementation status: in progress as bridge-release preparation; no release has been published. The deployed Python updater prefers OS/architecture-specific native Linux assets, performs an intentional same-version Python-to-Go migration, falls back to a newer transition ZIP, safely extracts native tar archives, validates manifest-v2 metadata and executable digests, and installs the native core for both ordinary and externally managed historical layouts. Automated two-cycle selection, successful migration, hostile-path quoting, and failed-health rollback simulations preserve settings, opaque databases, TLS material, service definitions, helpers, and legacy Python fallback files byte-for-byte. Published packages 2.1.1, 2.4.3, and 2.6.3 provide byte-pinned historical generations for the standard, externally managed, and runit-aware paths. Windows acceptance proves live Go-to-Python-to-Go ownership handoff over a 332-record final history and separately proves native archive installation, real-CLI package mode, forced health rollback, byte-identical program restoration, unchanged persistent state, restart, and package mode after rollback. Manual recovery is documented. Native artifact publication and remaining clean-machine/platform matrices remain.
